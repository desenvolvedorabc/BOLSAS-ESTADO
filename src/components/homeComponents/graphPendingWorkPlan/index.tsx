...
import { useGetIndicatorsPendingWorkPlan } from 'src/services/indicadores';
import { Loading } from 'src/components/Loading';

export function GraphPendingWorkPlan({ year, regional, cities }) {
  const { theme } = useContext(ThemeContext);
  const [seriesData, setSeriesData] = useState([]);
  const [isEmpty, setIsEmpty] = useState(false);

  const { data: indicators, isLoading: isLoadingIndicators } =
    useGetIndicatorsPendingWorkPlan(
      {
        year: year,
  ...
  const options = {
    ...
    tooltip: {
      ...
      useHTML: true,
      headerFormat:
        '<table><tr><th colspan="2" style="text-align: center">Bolsistas Pendentes</th></tr>',
      pointFormat:
        '<td><br/></td>' +
