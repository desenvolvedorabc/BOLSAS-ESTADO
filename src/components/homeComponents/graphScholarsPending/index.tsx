...
import { useGetIndicatorsScholarsPending } from 'src/services/indicadores';
import { Loading } from 'src/components/Loading';

export function GraphScholarsPending({ year, regional, cities }) {
  const { theme } = useContext(ThemeContext);
  const [seriesData, setSeriesData] = useState([]);
  const [isEmpty, setIsEmpty] = useState(false);

  const { data: indicators, isLoading: isLoadingIndicators } =
    useGetIndicatorsScholarsPending(
      {
        year: year,
